...
import { user, bank, myaccount } from "./Bank.js";
const userList = [{ userid: "kamran", pin: 1234, name: "kamran" }];
let atm = async () => {
    ...
    let objBank = new bank(userList);
    let userObj = new user(answers.userid, answers.userpin);
    let loginuser = objBank.authenticateuser(userObj);
    if (loginuser != undefined) {
        if (loginuser.userid !== "" && loginuser.pin != 0) {
            let bankaccount = new myaccount(loginuser);
